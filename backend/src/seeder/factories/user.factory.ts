...
define(User, (faker, ctx?: { role?: UserRole; tokens?: boolean }) => {
   ...
   user.email = faker.random.boolean() ? faker.internet.email(firstName, lastName, 'faked.com').toLowerCase() : faker.internet.email(user.username, 'faked.com').toLowerCase()
